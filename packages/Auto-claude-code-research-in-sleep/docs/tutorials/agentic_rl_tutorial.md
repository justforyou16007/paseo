...
其中 $\tau$ 是 trajectory（包含所有 $T$ 个 token / step），$m_t \in \{0, 1\}$ 是 agent action*mask（agent 自己生成的 token 为 1，observation/system token 为 0），$\rho_t = \pi*\theta(a*t \mid s_t) / \pi*{\theta\_\text{old}}(a_t \mid s_t)$ 是 token-level importance ratio。注意外层 expectation 索引是 trajectory $\tau$，内层 sum 索引是 token $t$，不能混淆。
