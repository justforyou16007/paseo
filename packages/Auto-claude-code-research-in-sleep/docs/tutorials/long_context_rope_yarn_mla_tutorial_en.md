...
- Compatible with RoPE / YaRN; per §10.3, the cache should store K _before_ RoPE, and at each forward re-apply RoPE based on the **current logical position** of each token in sink / window (some vendor implementations equivalently treat the sink segment as "fixed rotation + attention key index shift", with approximate effect)
