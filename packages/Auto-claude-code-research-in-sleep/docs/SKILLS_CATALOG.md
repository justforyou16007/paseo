...
- `Role` is a one-line summary — see the `SKILL.md` for the full contract,
  phases, and triggers.
- `Requires` lists external dependencies beyond ARIS core (Paseo MCP, Gemini
...
| Skill                                                                      | Role                                                                                                                                                                                                                                 | Requires                            |
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------- |
| [`/aris-setup`](../skills/aris-setup/SKILL.md) | The single human entry point for project configuration — reports the six setup stages, routes each unfinished one to the skill that finishes it, and seals the root charter | A second machine for the tester (ssh-reachable, Paseo daemon, `claude`) |
...
| [`/auto-review-loop`](../skills/auto-review-loop/SKILL.md)                 | **Workflow 2** — autonomous review → fix → re-review until positive or max rounds; uses the Paseo codex reviewer                                                                                                                           | Paseo MCP                           |
| [`/paper-writing`](../skills/paper-writing/SKILL.md)                       | **Workflow 3** — paper-plan → paper-figure → illustration → paper-write → paper-compile → auto-paper-improvement-loop                                                                                                                | Paseo MCP, LaTeX                    |
| [`/rebuttal`](../skills/rebuttal/SKILL.md)                                 | **Workflow 4** — parse reviews → atomize → strategy → draft → safety check → stress test → 2-version output → follow-ups                                                                                                             | Paseo MCP                           |
| [`/resubmit-pipeline`](../skills/resubmit-pipeline/SKILL.md)               | **Workflow 5** — text-only port across venues (no new experiments, no bib edits) — isolation → anonymity → audits `--soft-only` → microedit → kill-argument gate → compile + push                                                    | Paseo MCP, LaTeX                    |
| [`/paper-talk`](../skills/paper-talk/SKILL.md)                             | **Workflow 6** — paper → slide outline → Beamer + PPTX → per-page polish → assurance audits → final report                                                                                                                           | Paseo MCP, LaTeX, python-pptx       |
| [`/research-refine-pipeline`](../skills/research-refine-pipeline/SKILL.md) | Sub-pipeline used by `/idea-discovery` — refine method + plan experiments in one chain                                                                                                                                               | Paseo MCP                           |
| [`/patent-pipeline`](../skills/patent-pipeline/SKILL.md)                   | Full patent drafting — invention → claims → spec → jurisdiction format (CN / US / EP)                                                                                                                                                | Paseo MCP                           |
...
| [`/auto-research-loop`](../skills/auto-research-loop/SKILL.md)             | **Metric-target iteration loop** - one iteration is research-pipeline Stage 1-3 (idea-discovery -> experiment-bridge -> auto-review-loop) plus a deterministic metric gate; iteration 1 reproduces the baseline the brief describes, the orchestrator writes nothing to the wiki, and on stop the run exports its best iteration from the wiki as `result-package.json`                                                             | Paseo MCP, research-wiki            |
| [`/meta-optimize`](../skills/meta-optimize/SKILL.md)                       | **Workflow M** — analyze ARIS usage logs and propose SKILL.md / prompt / default-parameter improvements (outer-loop self-evolution)                                                                                                  | Paseo MCP, hook logging             |
| [`/meta-apply`](../skills/meta-apply/SKILL.md)                             | **Privileged landing gate** — the only skill allowed to mutate the skill corpus; lands `/meta-optimize` patches the human approved, after a fresh cross-model jury PASS on the staged diff (read-only producer ≠ privileged applier) | Paseo MCP, human-in-loop            |
...
| [`/alphaxiv`](../skills/alphaxiv/SKILL.md)                 | Quick single-paper lookup via [AlphaXiv](https://alphaxiv.org) at one explicitly selected depth                                         | None                                           |
...
| [`/novelty-check`](../skills/novelty-check/SKILL.md)       | Verify a research idea is novel against recent literature — multi-source search + cross-model verification + closest-prior-work table           | Paseo MCP                                      |
...
| Skill                                                          | Role                                                                                                                           | Requires                  |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------- |
| [`/idea-creator`](../skills/idea-creator/SKILL.md)             | Brainstorm 8-12 ideas, filter by feasibility, pilot on GPU, rank by signal                                                     | Paseo MCP, GPU for pilots |
| [`/research-refine`](../skills/research-refine/SKILL.md)       | Iterative method refinement — problem anchor → up to 5 review rounds → score ≥ 9                                               | Paseo MCP                 |
...
| [`/ablation-planner`](../skills/ablation-planner/SKILL.md)     | Design ablation studies from a reviewer's perspective (after main results pass `/result-to-claim`)                             | Paseo MCP                 |
...
| Skill                                                          | Role                                                                                                                                                                                                                                                                                                                                                                                  | Requires                            |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| [`/experiment-env-manager`](../skills/experiment-env-manager/SKILL.md) | Sole entry point for experiment environment lifecycle — baseline creation, runtime error handling, on-demand audit; dispatches env-configuration for generation and env-audit for validation; repair loop until pass or human intervention | Paseo MCP |
| [`/experiment-env-configuration`](../skills/experiment-env-configuration/SKILL.md) | Configure experiment environment once, freeze prepare→run→collect→analyze into a reusable project-local skill with 7 atomic scripts; cross-model audited                                                                                                                                                                                                                              | Paseo MCP                           |
| [`/experiment-env-audit`](../skills/experiment-env-audit/SKILL.md) | Cross-model audit of experiment environment config — checks command provenance, metric agreement, failure detectability, environment reachability, dynamic modification simulation; dispatched exclusively by env-manager | Paseo MCP |
| [`/run-experiment`](../skills/run-experiment/SKILL.md)         | Deploy experiments to local / remote / Vast.ai / Modal GPU                                                                                                                                                                                                                                                                                                                            | GPU (configurable)                  |
| [`/analyze-results`](../skills/analyze-results/SKILL.md)       | Iterative experiment analysis driver — runs analyze.sh, dispatches cross-model verifier for completeness, iterates (modify scripts / trigger supplementary experiments) until analysis is thorough                                                                                                                                                                                    | None                                |
| [`/analysis-wandb`](../skills/analyze-results-tools/analysis-wandb/SKILL.md) | W&B data retrieval for analysis sub-skills (export + run state)                                                                                                                                                                                                                                                                                                                   | W&B account                         |
| [`/analysis-convergence`](../skills/analyze-results-tools/analysis-convergence/SKILL.md) | Convergence / divergence / NaN / entropy-collapse checks against frozen thresholds (verdict proposal only)                                                                                                                                                                                                                                                                        | None                                |
| [`/analysis-training-dynamics`](../skills/analyze-results-tools/analysis-training-dynamics/SKILL.md) | Training-dynamics interpretation: loss curves, LR schedule, train/eval gap, gradient norms                                                                                                                                                                                                                                                                                         | None                                |
| [`/analysis-comparison`](../skills/analyze-results-tools/analysis-comparison/SKILL.md) | Cross-experiment comparison tables, multi-seed statistics, sweep trends, outlier flags (Type-A)                                                                                                                                                                                                                                                                                    | None                                |
| [`/analysis-probe`](../skills/analyze-results-tools/analysis-probe/SKILL.md) | Deep analysis that measures something new: offline recompute, instrumented probe reruns, checkpoint replay, controlled probes — hypothesis-first, evidence-keyed ledger, no metric authority | GPU (probe reruns) |
...
| Skill                                                        | Role                                                                                                                                                                      | Requires                  |
| ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| [`/research-review`](../skills/research-review/SKILL.md)     | Single-round deep critical review from external LLM (Paseo codex GPT xhigh by default; `oracle-pro` route for Pro tier)                                                         | Paseo MCP (or Oracle MCP) |
| [`/experiment-audit`](../skills/experiment-audit/SKILL.md)   | Cross-model integrity check of experiment code + results — catches fake ground truth, score-normalization fraud, phantom results, scope inflation                         | Paseo MCP                 |
| [`/result-to-claim`](../skills/result-to-claim/SKILL.md)     | Map experimental results to intended claims — judges what's supported, what's not, what's missing; routes to next action                                                  | Paseo MCP                 |
| [`/paper-claim-audit`](../skills/paper-claim-audit/SKILL.md) | Zero-context numeric verification — every number / comparison / scope claim in the paper checked against raw result files by a fresh reviewer (no confirmation bias)      | Paseo MCP                 |
| [`/citation-audit`](../skills/citation-audit/SKILL.md)       | Bibliography audit — existence + metadata correctness + context appropriateness for every `\cite{}`; `--soft-only` mode for frozen-bib resubmits                          | Paseo MCP, web access     |
| [`/proof-checker`](../skills/proof-checker/SKILL.md)         | Rigorous mathematical proof verification — 20-category issue taxonomy, two-axis severity, side-condition checklists, counterexample red team, proof-obligation ledger     | Paseo MCP                 |
| [`/kill-argument`](../skills/kill-argument/SKILL.md)         | Two-thread adversarial review — Thread 1 writes the strongest 200-word rejection memo; Thread 2 (independent) defends point-by-point and surfaces still-unresolved issues | Paseo MCP                 |
...
| [`/paper-compile`](../skills/paper-compile/SKILL.md)                             | Compile LaTeX paper to PDF once and verify submission readiness; compilation errors stop the phase                                                                 | LaTeX (`latexmk`, `pdfinfo`)                 |
| [`/auto-paper-improvement-loop`](../skills/auto-paper-improvement-loop/SKILL.md) | 2-round content review + format check — typical 4 / 10 → 8.5 / 10 score lift. `--edit-whitelist` mode for resubmits                                                   | Paseo MCP                                    |
...
| [`/slides-polish`](../skills/slides-polish/SKILL.md)         | Per-page Paseo codex review + targeted python-pptx / Beamer fixes (font scaling, frame resize, banner-as-tcolorbox, italic leak guard, em-dash spacing, CJK font hint, anonymity placeholder discipline)                   | Paseo MCP, python-pptx         |
...
| [`/patent-novelty-check`](../skills/patent-novelty-check/SKILL.md)     | Assess patent novelty and non-obviousness against prior art (patentability evaluation) | Paseo MCP         |
| [`/patent-review`](../skills/patent-review/SKILL.md)                   | External patent-examiner-style critical review of a patent application                 | Paseo MCP         |
...
| Skill                                                              | Role                                                                                                                                                                                                        | Requires                                 |
| ------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| [`/research-wiki`](../skills/research-wiki/SKILL.md)               | Persistent research knowledge base — papers / ideas / experiments / claims / problems with typed relationships. Workflow hooks auto-ingest across the research lifecycle; `export_result_package` picks a stopped run's best iteration out of the wiki and writes its result package                                               | Node.js compiled helper                |
| [`/wiki-enrich`](../skills/wiki-enrich/SKILL.md)                   | Fill per-paper TODO sections from one explicitly selected source; a failed source stops the batch and never switches source automatically | WebFetch + selected source helper                 |
| [`/render-html`](../skills/render-html/SKILL.md)                   | Render ARIS Markdown / JSON artifacts into reviewed single-file HTML views for human reading                                                                                                                | Node.js compiled helper; Paseo MCP for review gate |
...
2. Per the
   ...
   the canonical `.aris/dist/` → `dist/` runtime lookup — do not add a second
   helper, source, model, or command when the selected helper fails.
