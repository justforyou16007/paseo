...
Optional external-review MCP bridge for ARIS, using the direct Gemini API by default.

## What it does

- Uses **Gemini** as the explicitly selected external reviewer
- Use this bridge only when the caller explicitly selects Gemini as the
  reviewer backend
...
cp mcp-servers/gemini-review/server.py ~/.claude/mcp-servers/gemini-review/server.py
claude mcp add gemini-review -s user --env GEMINI_REVIEW_BACKEND=api -- python3 ~/.claude/mcp-servers/gemini-review/server.py
```
...
- `GEMINI_REVIEW_BACKEND`: reviewer backend override, one of `api`, `cli`, or `agy`; defaults to `api`
...
- `GEMINI_REVIEW_BACKEND=agy` routes review prompts through `agy --print` when you explicitly select that backend. The bridge still requires the final review text on stdout and uses the per-call log only to audit the conversation and selected model.
...
- Antigravity provenance validates conversation ids with `^[0-9A-Za-z_-]+$`, caps ids at 128 characters, and requires the per-call invocation nonce to bind the log and transcript to the current request.
...
- `imagePaths` / `image_paths` are supported only by the direct Gemini API backend in this bridge. Paths must resolve under `GEMINI_REVIEW_WORKSPACE_ROOT` and are opened with no-symlink safeguards; platforms where the opened fd cannot be re-verified fail closed. The explicitly selected CLI and `agy` backends are text-only.
...
- the async path completed `review_start` -> `review_status`, and threaded `review_reply_start` -> `review_status` completed with the same explicit backend
