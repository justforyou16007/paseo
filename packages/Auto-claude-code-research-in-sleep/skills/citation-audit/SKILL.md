...
allowed-tools: Bash(*), Read, Grep, Glob, Edit, Write, mcp__paseo__create_agent, mcp__paseo__send_agent_prompt, mcp__paseo__list_pending_permissions, mcp__paseo__respond_to_permission, mcp__paseo__list_agents, mcp__paseo__get_agent_status, mcp__paseo__archive_agent, WebSearch, WebFetch, mcp__paseo__create_heartbeat, mcp__paseo__delete_heartbeat
...
> on a timer adds no new signal (it changes only when the _bibliography_
> changes). Schedule the _external wait that precedes it_ — bibliography
