...
> attack changes only when the _paper_ changes). Schedule the _external wait
> that precedes it_ — draft stable → then run this **once** before submission.
> See
...
**Default OFF.** The deliverable of this skill _is_ a verdict — the single
...
forcing one paragraph produces sharper feedback than a balanced list (see _Why
This Exists_). Do **not** replace it with a list.

Under `beast` effort you may widen the _evidence_ the commitment draws on
...
1. **Axis probes (evidence breadth).** Run the six attack axes (theorem
   ...
   each asked for the strongest ~120-word thrust _on that axis alone_. These
