...
> signal, full token cost. Schedule the _external wait that precedes it_ (work
...
_For manual backend:_ use `mcp__manual_review__review` with the same brief
...
> (see _Review Tracing_ below — that audit copy is kept in every mode); fold the review
> _conclusions_ (consensus, claims matrix, prioritized TODOs) into the orchestrator's
