...
(`references/` test data + `tool-unit-test.py`) via a fresh paseo codex
sub-agent — work and verifier are different families
([`experiment-integrity.md`](../shared-references/experiment-integrity.md)).
The Paseo MCP grant (`mcp__paseo__create_agent`) lives on the subagent's own frontmatter
...
executor could not have written. The Paseo MCP capability (`mcp__paseo__create_agent`) for that
cross-model test authorship (fresh sub-agent, file paths only, never continue an existing agent
...
- [`../../agents/experiment-analysis-agent.md`](../../agents/experiment-analysis-agent.md)
  ...
  loop (work = Claude, verification = Codex). Granted Paseo MCP (`mcp__paseo__create_agent`) on its
  own frontmatter.
...
- [`shared-references/reviewer-routing.md`](../shared-references/reviewer-routing.md)
  — Codex reviewer (`gpt-5.5`, `xhigh`) dispatched via paseo codex sub-agent, fresh agent.
