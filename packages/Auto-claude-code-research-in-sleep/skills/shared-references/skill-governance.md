...
questions must have a recorded, checkable answer _before_ that artifact is allowed
...
- **A provenance record cannot be same-family.** `stamp()` calls
  ...
  **openai** family, not a separate one). You therefore _cannot produce_ a valid
...
- **meta-optimize** — when an auto-patch lands (Step 6), `stamp()` the changed
  ...
  _refusing_ is the last line of defense: if the patch was never cross-model
...
- **acceptance-gate** — the cross-family assertion here is the _enforcement_ of the
...
there it is a _captured claim_ that hardens into a self-cited falsehood; here it is
a _captured skill_ that an unreviewed loop grants itself authority to keep. ARIS's
increment is to make cross-family a **structural assertion on the stamp's content**:
`assert_cross_family` genuinely raises, so you cannot produce a _valid_ same-family stamp
(not a config flag you can flip). The scope is precise — this governs _what a stamp can
say_, not _whether a write calls `stamp()` at all_. Whether a corpus mutation is stamped
