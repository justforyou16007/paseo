...
- **Layer 1** blocks overt injection. Its strength is that it has no model: a
  poisoned model can't _talk its way past a regex_. It is the cheap pre-filter.
...
A clean layer-1 scan means only _"no known-bad strings"_, never _"safe"_. This
...
broad _detection_ there, but _blocking_ is reserved for paths where a human can
...
- **A readable file** (MEMORY.md, a wiki page): keep the file as-is on disk;
  quarantine only the _injected view_ at load time.
...
- **To extend** (same helper, same scopes): MEMORY.md write + load; fetched
  ...
  _SKILL.md scanning needs tuning first:_ legit ARIS skill docs say things like
...
> Hermes scans memory/context injection but leaves _learned-content correctness_
