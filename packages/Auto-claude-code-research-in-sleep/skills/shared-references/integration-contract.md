...
Rule of thumb: **SKILL.md prose can _describe_ an integration; it cannot
_guarantee_ one.** Any integration whose silent failure would damage the
...
the `meta_opt/` hook scripts, and the generated project-local experiment ops — are out of scope.
...
| `run-state.js` (in `/auto-research-loop`)                                                                                 | A (gate)                                                                                                                           | The iteration loop requires crash-safe resume; without run-state, acceptance provenance is lost and resume cannot determine which phase to continue from. Block if unresolved.                                                                                                                                                                                                                                                                                                                                                                               |
| `metric-gate.js` (in `/auto-research-loop`)                                                                               | A (gate)                                                                                                                           | Parses `## Metric Target` from CLAUDE.md (`config`) and evaluates the deterministic stop gate (`evaluate`). The loop cannot start without a validated metric config and cannot decide stop/continue without the gate. Block if unresolved.                                                                                                                                                                                                                                                                                                                  |
| `dashboard-merge.js` (in `/auto-research-loop`)                                                                          | A (gate)                                                                                                                           | Validates run/manifest/phase ownership and worker field permissions, then atomically and idempotently merges a receipt. Without it, an unrelated or malformed receipt could overwrite loop state, and resume could double-apply metric history. Block if unresolved.                                                                                                                                                                                                                                                                                          |
| `render_w_agent_prompt.sh` (in `/research-pipeline`, `/auto-research-loop`)                                               | A (gate)                                                                                                                           | Emits the per-run `paseo-config.json` (`--emit-config`); every `create_agent` reads provider/mode/thinking from it. Without it, sub-agent dispatch has no provider config. Block if unresolved. Shell helper: `.aris/tools/` → `tools/`.                                                                                                                                                                                                                                                                                                                    |
...
| `experiment-env/env-helper.js` `provision` / `preflight` / `sync` / `deploy` / `monitor` / `collect` / `destroy`          | A (gate, multi-owner)                                                                                                              | Unified experiment-environment control across local/remote/vast/modal; consumed by the generated experiment skill's ops, `/vast-gpu`, `/serverless-modal`, `/experiment-queue`. Canonical location: `dist/tools/experiment-env/` (compiled from `src/tools/experiment-env/`). A failed action subcommand blocks the calling skill.                                                                                                                                                  |
