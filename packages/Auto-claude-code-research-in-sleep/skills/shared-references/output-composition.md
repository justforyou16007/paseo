...
2. Inlines each sub-skill's returned findings into the canonical deliverable (or into
   ...
   which the report _links to_, not copies).
...
- [`output-versioning.md`](output-versioning.md) — _how_ to write a file you do write
  (timestamp + fixed-name copy). Composition decides _whether_ a sub-skill writes a
