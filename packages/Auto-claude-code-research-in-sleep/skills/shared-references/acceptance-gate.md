...
single-shot experiment judging; this one covers the _recurring verdict_
...
The loop may freely _drive_ itself toward a target — schedule the next
config, recompile, re-run the failed job, spawn ten search branches.
What it may not do is _acquit_ its own work — declare the paper good,
...
A machine-checkable or externally-observable signal of _what happened_,
with no judgment of _merit_. Claude **MAY** self-judge Type-A gates —
...
Type-A gates are _coverage and completion_ facts. Claude self-judging
...
A judgment of _merit, correctness, or sufficiency_. Claude must
...
- ❌ "score >= 6" — when _Claude_ assigned the score
...
> _Could a dumb script with no taste answer this gate?_
>
...
is nothing _but_ taste — Type-B. "The job exited 0" — Type-A. "The job's
...
B-part. `/auto-review-loop`'s real condition is _"score >= 6 AND verdict
contains 'ready'"_ evaluated each round — but the **score and the
...
B-part doesn't disappear because it's inconvenient; it gets _routed_.
...
3. **For every Type-A gate:** Claude may self-judge. Prefer an
   _external_ check where one exists (read an exit code, stat a file,
...
loop is _designed_ (by this contract) so that removing the external
...
- **The execution loops (dse, auto-debug, queue) are Type-A all the way
  ...
  They are _correctly_ allowed to self-acquit, because there is nothing
  of merit being judged: a converged number from a real simulator is an
  observation, not an opinion. (The moment someone adds _"...and the
  result is good enough to claim"_ to a dse stop condition, that clause
...
`/dse-loop` optimizes a metric the benchmark _itself_ produces (cycles,
...
So dse may self-terminate on _"best config found within budget"_ (A), but
the claim _"and this is a publishable result"_ leaves the loop and goes
...
- ✅ Ten Claude branches each _attempting_ a different search query, then
...
- ❌ N Claude reviewers each scoring the paper, then taking the
  **majority/average as the accept verdict.** This _feels_ like a jury
  ...
  family**, not a _bigger N of the same one_.
...
> _family diversity_, not _sample count_. Fan-out scales breadth and
...
2. **Every Type-B gate routes to a cross-model verdict** per
   `reviewer-routing.md`; the loop's continue/stop reads _that_ verdict,
...
- **"The loop decides when it's good enough."** Good-enough is Type-B;
  the loop may decide when it's _done running_, not when it's _good_.
- **"We re-review until it passes."** Fine — but _who_ says it passed? If
...
- **"Score >= 6, so stop."** Only safe if a _different family_ assigned
...
value is specific and bounded: a reviewer from a different model family breaks _correlated_
...
Treat a PASS as the strongest _automatable_ heterogeneous quality check this framework has, then keep the human in the loop for
