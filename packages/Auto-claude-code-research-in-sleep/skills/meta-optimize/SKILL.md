...
- **No apply step.** There is no in-skill "apply the patch" path (see Step 6). The
  producer ends by _staging_ approved patches for `/meta-apply`; a human must then
...
- **`Bash` writes to the corpus are filtered, not impossible — be honest about the
  layers.** What IS fully closed: the _accidental / in-flow_ self-acquittal — this skill
  ...
  NOT a complete sandbox** — a _deliberately_ obscured Bash write (`git apply`, `patch`,
...
- **Anti-self-poisoning screen** (see [`shared-references/capture-antipatterns.md`](../shared-references/capture-antipatterns.md)):
  ...
  Encode the _fix / the flag needed / the workaround_, not "X can't do Y".
...
2. Tell the user: _"Staged M patches. Run `/meta-apply` to judge & land them."_
