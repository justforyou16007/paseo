...
> **Markdown is for writers. HTML is for readers.** ARIS workflow nodes write Markdown (canonical, audit-trail-friendly, machine-parseable). `/render-html` turns _selected_ artifacts into a polished single-file HTML view for the human who actually has to read them. The Markdown stays the source of truth.
...
```json
{
  ...
  "summary": { "template": "<academic|dashboard>", "source_file": "<input path>" },
  "dashboard_patch": {
    "html_rendered": true
  },
