...
the next gate flags _that_, the agent invents another element…". Pinning the component set turns
...
| Field                        | Meaning                                                                                                                          |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| **Purpose**                  | The single job this component does. If your content needs a different job, you need a _different_ component, not a restyled one. |
...
- **Allowed fix operations**: (c) AR-bandwidth width adjust, (f) `.figure--wide`, (g) asset fix
  (re-crop, swap for a sharper figure from the _same_ paper, re-run preprocess). Width changes
...
- **Purpose**: The single dominant visual on the hero template — one large figure / system
  diagram / table that _is_ the poster's message. Replaces the framework banner.
...
- **Allowed variants**: `.callout` (solid `--accent`, white text, `<strong>` in gold) and
  ...
  gold _gradient_ to a flat fill per §E.2). No third variant.
...
- **Anti-patterns**: a logo _without_ `data-color-exempt="logo"` (rule 1 — its colors leak into
...
- **Contract**: the table caption or setup line MUST state the derivation ("Δ = AuxDPO −
  DPO, derived"). Negative/degrading values use _italic_, never red.
