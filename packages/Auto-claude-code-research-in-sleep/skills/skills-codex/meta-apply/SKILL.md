...
**read-only** — no `Write`/`Edit`, no apply step; they can only _stage_ candidate patches
...
xhigh`, read-only, paths-only per
   ...
   staged `.diff` + its target. Ask: _does this change improve the harness without
...
> The producer may have written an _advisory_ pre-screen into the manifest to help the
...
A stamp records that a change passed _a process_ (cross-model jury at landing + human
landing), not that it is _correct_. To prevent "approved-but-wrong with a stamp that
