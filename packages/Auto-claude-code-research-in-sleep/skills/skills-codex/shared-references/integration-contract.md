...
| `analysis_tools.py` `register` / `deprecate` / `add-category` (in `/analyse-tool`)                                        | B (side-effect)                                                                                                 | Personal convenience collection of reusable analysis methods (`$HOME/.aris/analysis_tools/`, overridable via `$ARIS_ANALYSIS_TOOLS_DIR`); primary research output delivered without it (warn-and-skip). Personal — not repo-local — because skills cannot be reloaded at runtime so the helper _is_ the access mechanism. Register enforces mandatory `scripts/tool-unit-test.py` + `references/` test-data contract; `test --slug` is the Type-A deterministic gate the subagent runs first. |
