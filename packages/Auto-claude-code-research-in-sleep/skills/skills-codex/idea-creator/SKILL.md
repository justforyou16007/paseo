...
Generate ideas **inline in the current session** — the executor (Codex) does the
generation. Generation is candidate production (DRIVE, Type-A work), not a
verdict, so same-family is safe per `acceptance-gate.md`. Reserve cross-model
spawns exclusively for the Phase-4 quality/novelty verdict.

Write the full brainstorming prompt to `idea-stage/executor_brainstorm_bundle.md`
and then generate inline:
...
Write the final list to idea-stage/generated_ideas.json.
```

Save the generated ideas list path for Phase 3 consolidation.

Save a Review Tracing record for this generation pass following `../shared-references/review-tracing.md`, including the landscape summary, prompt summary, raw idea list path.
