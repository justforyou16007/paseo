...
review rounds making it _worse_ — each round added a new badge color or bespoke SVG
...
1. Read the paper source (`.tex` ideal; PDF otherwise). Extract: title/authors/affils,
   ...
   dense mode. Bullets ≤ 8 words when possible — density comes from _structure_, not
...
may waive the _total-area_ rule (`--waive-total-area`) at a human checkpoint — never
silently. Never draw bespoke decorative SVG "figures" as substitutes.

**Figure-area bands** (asset gate, fractions of _body_): total target **14–22 %**
...
- `.poster` dims to the venue canvas (two edits, same values); apply the chosen token
  ...
  (A fresh scaffold is _expected_ to fail `measure` — that gate judges a filled poster.)
...
Balance under-filled columns with _content from the paper_ (Gate C), never with
...
columns, trim/grow text _from the paper_, resize a figure within its AR band);
