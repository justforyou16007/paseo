...
>    quantifiers) is structural extraction. Whether a proof step is _valid_ —
...
>    — the loop may self-verify _that the ledger is complete_, never _that the
>    proofs are correct_.
...
>      mathematically valid. A shard's `UNVERIFIED` means _"the paper cites no
>      discharge location"_, NOT _"the shard checked the math and it fails"_.
...
> verdict field. 2. **Global artifacts are a barrier, computed on the merged ledger, not
