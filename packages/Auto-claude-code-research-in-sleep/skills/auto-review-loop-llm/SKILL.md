...
> change — zero new signal, full token cost. Schedule the _external wait that
> precedes it_, not the verdict. See
...
1. **Check `review-stage/REVIEW_STATE.json`** for recovery _(fall back to `./REVIEW_STATE.json` if not found — legacy path)_
