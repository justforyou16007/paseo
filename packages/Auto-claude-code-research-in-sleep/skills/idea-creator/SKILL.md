...
same-family _acquittal_ is not).
...
### Phase 2: Idea Generation (executor-driven brainstorm)

Use the **executor** (Claude) for idea generation, never the reviewer. Generation
is candidate production — a DRIVE activity, not a verdict — so same-family is
safe per `shared-references/acceptance-gate.md` (idea generation is Type-A work;
only the _acquittal_ of quality/novelty in Phase 4 requires cross-model).
Reserve the reviewer's scarce cross-model capacity for the Phase-4 jury.

Dispatch a paseo claude sub-agent (fresh) per `shared-references/paseo-subagent-dispatch.md`
to generate ideas from the landscape + gap survey. Write the full brainstorming
request to `idea-stage/claude_brainstorm_bundle.md`, then keep the sub-agent's
prompt short:
...
mcp__paseo__create_agent:
  ...
  initialPrompt: |
    ...
    idea-stage/claude_brainstorm_bundle.md> and follow all instructions in it.
  notifyOnFinish: true
```
...
If paseo MCP is unavailable (fallback), run the brainstorm inline in the
...
2. **Novelty signal — ANNOTATE, do not eliminate**: for each surviving idea,
   ...
   related, with links). This is _input for the jury_, not a filter. The
...
3. **Impact signal — ANNOTATE, do not eliminate**: attach a one-line
   ...
   _precisely_ the question the Phase-4 cross-model devil's-advocate asks.
...
**Lead every recommended idea with its method, in plain language.** Before any hypothesis, novelty score, or claim, state in 2–4 concrete steps what we actually build / train / run — no jargon, no claim-IDs. The reader must understand _what we do_ before _what we claim_; claims (hypothesis, validation, expected outcome) come after and read as the method's acceptance criteria.
