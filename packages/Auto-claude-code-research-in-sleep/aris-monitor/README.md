...
- it is inherently **transient** — the widget **polls every ~2 s** to catch it.
  On a quiescent machine you will only ever see _working_ / _done_, never
  _waiting_. That is expected, not a bug.
...
> **Note on `run.sh` and venvs:** ARIS-Monitor has _no_ third-party
...
Cmd-Tab-able, and can sit _below_ a true-fullscreen app's Space; the look is
