...
> 📖 _Preview from the [Diffusion Foundations cheat sheet](https://wanshuiyin.github.io/ARIS-in-AI-Offer/tutorials/diffusion_foundations_tutorial.html) — every tutorial in [ARIS-in-AI-Offer](https://github.com/wanshuiyin/ARIS-in-AI-Offer) follows the same three-pillar structure (foundations / interview Q&A / runnable code)._
>
> 🌐 _Same workflow, different deliverable — [**ARIS-Homepage v1 live demo**](https://wanshuiyin.github.io/) (CV → fact-checked single-file academic homepage via `/homepage-generator`)._
>
> 📝 _Three long-form blogs, cross-model collaborative writing via `/render-html` — [Continuous DLM — a representation-perspective survey (2026 H1)](https://wanshuiyin.github.io/ARIS-in-AI-Offer/blogs/continuous_dlm_representation_perspective.html) · [Cosmos 3 — understanding + generation in one Transformer (MoT)](https://wanshuiyin.github.io/ARIS-in-AI-Offer/blogs/cosmos3_mot_guide.html) · [Diffusion × representation × manifold learning](https://wanshuiyin.github.io/ARIS-in-AI-Offer/blogs/diffusion_representation_manifold.html)._
...
>
> _Think of it like adversarial vs. stochastic bandits: a single model self-reviewing is the stochastic case (predictable reward noise), while cross-model review is adversarial (the reviewer actively probes weaknesses the executor didn't anticipate) — and adversarial bandits are fundamentally harder to game._
>
...
ARIS reads the paper → finds its weaknesses → clones the codebase → generates ideas that specifically fix _those_ weaknesses with _that_ code → runs experiments → writes your paper. Like telling a research assistant: _"read this paper, use this repo, find what's missing, and fix it."_

> Mix and match: `ref paper` only = "what can be improved?", `base repo` only = "what can I build with this code?", both = "improve _this_ paper using _this_ code."
...
> _💡 From idea to paper to podium — one toolchain. 🌱_
...
- **2026-05-02** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🛠️🔗 **Tools-infrastructure migration started**. (a) `install_aris.sh` creates optional `.aris/tools` symlink ([#192](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/192), closes [#174](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/174)) — Phase 0 of the 4-step tools-stability plan (#174 → #176 → #177 → #178); idempotent, **zero impact until reinstall**. (b) [`/experiment-queue`](skills/experiment-queue/SKILL.md) orchestration paths repaired ([#193](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/193)) — first real user of the symlink; 7 cascading bugs fixed via 3 rounds of Codex MCP `gpt-5.5` xhigh audit. Pure prose + docstring; `queue_manager.py` logic untouched. Windows `install_aris.ps1` parallel update tracked as follow-up
...
- **2026-04-30** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🪞 **Codex skill mirror rebuilt + dedicated install/update chain** ([#179](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/179), community contribution by [@No-518](https://github.com/No-518)). `skills/skills-codex/` now mirrors all 67 mainline skills; replaces `mcp__codex__codex` reviewer path with Codex-native `spawn_agent` + `send_input`. New `tools/install_aris_codex.sh` + `tools/smart_update_codex.sh` handle project-local symlinks with manifest tracking. Anti-drift: `tests/test_codex_skill_mirror.py` + `tests/test_codex_install_update.py` (26 failure paths). Open discussion in [#184](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/184)
...
- **2026-04-10** — 🧠 **`tools/smart_update.sh`** — intelligent skill updater. Compares local vs upstream, detects personal customizations (server paths, API keys), only updates safe skills. Superseded by Paseo's automatic install; the script has since been removed.
...
```bash
# 1. Install skills — Paseo does this for you: adding a project copies
#    skills/ → .claude/skills/, tools/ → .aris/tools/, and writes the
#    .aris/installed-skills.txt manifest. Nothing to run.
#
#    Not using Paseo? Copy them in by hand:
git clone https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep.git ~/aris_repo
mkdir -p ~/your-project/.claude/skills
cp -r ~/aris_repo/skills/* ~/your-project/.claude/skills/
export ARIS_REPO=~/aris_repo          # so the dist/tools helper chain resolves
# (prefer a global install instead? cp -r ~/aris_repo/skills/* ~/.claude/skills/)

# 1b. Update later (when upstream changes)
cd ~/aris_repo && git pull
# Paseo installs: rm .aris/installed-skills.txt and .claude/skills/, then re-add the project
# Manual installs: re-run the cp -r above (overwrites local edits under .claude/skills/)
...
🎉 **Community Skills (14):** [research-refine](skills/research-refine/SKILL.md) · [experiment-plan](skills/experiment-plan/SKILL.md) · [research-refine-pipeline](skills/research-refine-pipeline/SKILL.md) · [grant-proposal](skills/grant-proposal/SKILL.md) · [paper-poster](skills/paper-poster/SKILL.md) · [paper-slides](skills/paper-slides/SKILL.md) · [mermaid-diagram](skills/mermaid-diagram/SKILL.md) · [proof-writer](skills/proof-writer/SKILL.md) · [comm-lit-review](skills/comm-lit-review/SKILL.md) · [dse-loop](skills/dse-loop/SKILL.md) · [idea-discovery-robot](skills/idea-discovery-robot/SKILL.md) · [formula-derivation](skills/formula-derivation/SKILL.md) · [paper-illustration](skills/paper-illustration/SKILL.md) · [writing-systems-papers](skills/writing-systems-papers/SKILL.md)
...
Unlike Workflows 1–4 which optimize _research artifacts_ (papers, code, experiments), Workflow M optimizes the _harness itself_ — the SKILL.md instructions, default parameters, and convergence rules that govern how ARIS operates. Inspired by [Meta-Harness](https://arxiv.org/abs/2603.28052) (Lee et al., 2026).
...
> 💡 **Paseo installs ARIS for you.** When you add a project in Paseo, the daemon copies each skill into `.claude/skills/<skill-name>` (flat, one directory per skill, so Claude Code's slash-command discovery picks them up), copies `tools/` into `.aris/tools/`, and writes a manifest at `.aris/installed-skills.txt` recording what it installed and where the ARIS repo lives. Existing files are never overwritten.

Confirm the install landed:

```bash
cd ~/your-paper-project
ls .claude/skills | wc -l          # number of installed skills
head -4 .aris/installed-skills.txt # version / repo_root / project_root / generated
```

The install is **one-shot** — a project that already has `.aris/installed-skills.txt` is skipped, so your local edits under `.claude/skills/` are never clobbered. To pick up new upstream skills:

```bash
cd ~/your-paper-project
rm -rf .aris/installed-skills.txt .claude/skills
# then re-add the project in Paseo — it reinstalls from the current ARIS source
```

**Why the copy and not a symlink:** the project owns its skills. A `git pull` in the ARIS repo does not silently change what a running project executes, and a project can be edited, committed, and shipped without a live dependency on a checkout path elsewhere on the machine.

<details>
<summary><b>Migrating from a legacy nested install (pre-2026-04-20)</b></summary>

Old installs put everything under a single `.claude/skills/aris/` directory, which Claude Code's slash-command discovery never scanned (it only looks one level deep). To move to the flat layout:

```bash
cd ~/your-project
rm -rf .claude/skills/aris          # or move it aside if you made local edits
rm -f .aris/installed-skills.txt
# then re-add the project in Paseo, or copy the skills in by hand (see below)
```
...
**Project-local copy by hand (when you are not using Paseo):**
...
export ARIS_REPO=~/aris_repo    # required: a hand copy writes no manifest, and the
                                # helper chain falls back to $ARIS_REPO/tools/
```
...
# Update with: cd ~/aris_repo && git pull && cp -r skills/* ~/.claude/skills/
```
...
> 💡 **New Claude Code versions** may not auto-create `~/.claude/skills/`. If using global install, create it first: `mkdir -p ~/.claude/skills/`. Paseo's project install creates the directories it needs automatically.
...
# Paseo-installed project: drop the manifest and the installed copy, then
# re-add the project in Paseo to reinstall from the updated source.
cd ~/your-project && rm -rf .aris/installed-skills.txt .claude/skills

# Manual / global install:
# cp -r ~/aris_repo/skills/* ~/.claude/skills/       # Option A: overwrite all
# cp -rn ~/aris_repo/skills/* ~/.claude/skills/      # Option B: only add new, keep yours
# cp -r ~/aris_repo/skills/experiment-bridge ~/.claude/skills/  # Option C: specific skill
```

> ⚠️ Updating **overwrites** skills you edited locally. If you customized a skill (server paths, API keys, lab-specific defaults), diff it against upstream first, or use Option B so only new skills are added.
