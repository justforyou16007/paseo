...
/**
 * A read-only surface for the AutoResearch Visualization SDK. Loads the research
 * wiki, experiment environment, and refine logs from within a workspace cwd and
 * emits structured payloads. All file access is scoped with resolveScopedPath so
  ...
  private async readMarkdownDirectory(
    ...
    const parsed = await Promise.all(
      mdNames.map(async (name) => {
        // Scope each child through resolveScopedPath so symlink escapes are caught
        // (consistent with the file-explorer pattern).
